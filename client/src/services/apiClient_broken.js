...
class ApiClient {
  ...
  // Upload photo to create virtual question
  async uploadQuestionPhoto(photoFile) {
    const formData = new FormData();
    formData.append('photo', photoFile);
    
    const response = await fetch(`${API_BASE_URL}/api/ai/upload-question-photo`, {
      method: 'POST',
      body: formData
    });
  ...
  async getVirtualQuestions() {
    const response = await fetch(`${API_BASE_URL}/api/ai/virtual-questions`);
  ...
  async checkAnswer(answerData) {
    const response = await fetch(`${API_BASE_URL}/api/ai/check-answer`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async checkSolutionPhoto(photoFile) {
    ...
    const response = await fetch(`${API_BASE_URL}/api/ai/check-solution-photo`, {
      method: 'POST',
      body: formData
    });
  ...
  async getSolutionAnalyses() {
    const response = await fetch(`${API_BASE_URL}/api/ai/solution-analyses`);
